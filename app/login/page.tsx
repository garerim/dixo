...
function detectWebView(): string | null {
  ...
  // Named app detection (user-agent patterns)
  ...
  // Android WebView
  if (/; wv\)/.test(ua) || /WebView/i.test(ua)) return "cette application";

  const isIOS = /iPhone|iPad|iPod/.test(ua);

  // iOS: no "Safari" in UA = definitely a WebView
  if (isIOS && !/Safari/.test(ua)) return "cette application";

  // iOS: has "Safari" but also has "Mobile/" without "Version/" = WKWebView
  if (isIOS && /Mobile\//.test(ua) && !/Version\//.test(ua)) return "cette application";
          ...
          {webViewApp ? (
            ...
            <>
              ...
              <div className="flex flex-col items-center gap-1 pt-2">
                <p className="text-xs text-muted-foreground">{t("webviewTrouble")}</p>
                <button
                  type="button"
                ...
                >
                  ...
                  {copied ? t("webviewCopied") : t("webviewCopyFallback")}
                </button>
              </div>
            </>
          )}
